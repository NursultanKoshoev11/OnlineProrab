...
func updateDailyReport(w http.ResponseWriter, r *http.Request, reportID string) {
	...
	err := appState.DB.Pool.QueryRow(ctx, `
		...
		SET report_date = $2, summary = $3, workers_count = $4, issues = NULLIF($5, ''), updated_at = now()
		WHERE id = $1
		RETURNING id::text, project_id::text, report_date::text, summary, workers_count, COALESCE(issues, ''), created_at::text
...
func deleteDailyReport(w http.ResponseWriter, r *http.Request, reportID string) {
	...
	result, err := appState.DB.Pool.Exec(ctx, `DELETE FROM daily_reports WHERE id = $1`, reportID)
	if err != nil {
...
func dailyReportProjectID(ctx context.Context, reportID string) (string, bool) {
	...
	err := appState.DB.Pool.QueryRow(ctx, `SELECT project_id::text FROM daily_reports WHERE id = $1`, reportID).Scan(&projectID)
