...
class BackendProjectDashboardScreen extends StatefulWidget {
  ...
  @override
  State<BackendProjectDashboardScreen> createState() =>
      _BackendProjectDashboardScreenState();
}

class _BackendProjectDashboardScreenState
    extends State<BackendProjectDashboardScreen> {
      ...
      body: FutureBuilder<ProjectDashboardData>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError)
            return BackendDashboardError(
              message: _friendlyError(snapshot.error),
              onRetry: _refresh,
            );
          final data =
              snapshot.data ??
              const ProjectDashboardData(expenses: [], reports: [], tasks: []);
            ...
            child: ListView(
              ...
              children: [
                ...
                if (data.expenses.isEmpty)
                  const DashboardEmptyLine('No expenses yet')
                else
                  ...data.expenses.map(
                    (item) => BackendExpenseTile(item: item),
                  ),
                const SizedBox(height: 20),
                DashboardSectionHeader(
                  title: 'Daily reports',
                ...
                if (data.reports.isEmpty)
                  const DashboardEmptyLine('No reports yet')
                else
                  ...data.reports.map((item) => BackendReportTile(item: item)),
                const SizedBox(height: 20),
                DashboardSectionHeader(
                  title: 'Tasks',
                ...
                if (data.tasks.isEmpty)
                  const DashboardEmptyLine('No tasks yet')
                else
                  ...data.tasks.map(
                    (item) => BackendTaskTile(
                      item: item,
  ...
  Future<void> _addExpense() async {
    final saved = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (_) => BackendExpenseFormScreen(
  ...
  Future<void> _addReport() async {
    final saved = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (_) => BackendReportFormScreen(
          projectId: widget.project.id,
  ...
  Future<void> _addTask() async {
    final saved = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (_) => BackendTaskFormScreen(
          projectId: widget.project.id,
...
class BackendExpenseFormScreen extends StatefulWidget {
  const BackendExpenseFormScreen({
  ...
  @override
  State<BackendExpenseFormScreen> createState() =>
      _BackendExpenseFormScreenState();
}
...
class BackendReportFormScreen extends StatefulWidget {
  ...
  @override
  State<BackendReportFormScreen> createState() =>
      _BackendReportFormScreenState();
}
