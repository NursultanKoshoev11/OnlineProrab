...
void main() {
      ...
      httpClient: MockClient((request) async {
        ...
        if (request.url.path == '/api/v1/auth/session') {
          ...
          return http.Response(
            jsonEncode({'refresh_token': 'refresh-123'}),
            201,
          );
        }
  ...
  test(
    'AuthRepository signOut revokes session and clears secure storage',
    () async {
      final store = SessionStore(storage: MemorySecureStore());
      await store.save(
        const SessionData(
  ...
  test(
    'AuthRepository throws AuthException when access token is missing',
    () async {
      final apiClient = ApiClient(
        httpClient: MockClient((request) async {
